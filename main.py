...
VAL_BATCH_SIZE = 64   # Number of images to generate embeddings of at one time
...
tuple_of_types = (tuple([tf.string]*3), tf.bool, tf.float32, tf.float32)
...
if WHICH_DATASET == 'paris':
    ...
    training_x = tf.data.Dataset.from_generator(pipe.dataset_maker, tuple_of_types, args = (PARIS_PATH_TXT,query_list,True, mean, std))
    validation_x = tf.data.Dataset.from_generator(pipe.dataset_maker, tuple_of_types, args = (PARIS_PATH_TXT,query_list,False, mean, std))
    ...
    training_x = tf.data.Dataset.from_generator(pipe.dataset_maker, tuple_of_types, args = (OXFORD_PATH_TXT,query_list,True, mean, std))
    validation_x = tf.data.Dataset.from_generator(pipe.dataset_maker, tuple_of_types, args = (OXFORD_PATH_TXT,query_list,False, mean, std))
...
dense_reduction_layer = Dense(4096)
...
def test_step(x):
    ...
    tick = time()
    emb.generate_all_embeddings(x, model, embed_path, VAL_BATCH_SIZE, mean, std)
    tock = time()
